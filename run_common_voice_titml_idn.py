...
def main():
    ...
    # Get the datasets:
    cv_train_dataset = datasets.load_dataset(
        "common_voice", data_args.dataset_config_name, split=data_args.train_split_name, cache_dir=model_args.cache_dir
    )
    cv_train_dataset = cv_train_dataset.remove_columns(['client_id', 'up_votes', 'down_votes', 'age', 'gender', 'accent', 'locale', 'segment'])
    titml_train_dataset = datasets.load_dataset(
        "titml_idn", split="train", cache_dir=model_args.cache_dir
    )
    assert cv_train_dataset.features.type == titml_train_dataset.features.type
    
    train_dataset = datasets.concatenate_datasets([titml_train_dataset, cv_train_dataset])
    eval_dataset = datasets.load_dataset("common_voice", data_args.dataset_config_name, split="test", cache_dir=model_args.cache_dir)
    eval_dataset = eval_dataset.remove_columns(['client_id', 'up_votes', 'down_votes', 'age', 'gender', 'accent', 'locale', 'segment'])
